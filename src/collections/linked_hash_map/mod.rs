// Disclaimer: code in this module was copied from https://github.com/contain-rs/linked-hash-map
...
///
/// ```
/// # #[macro_use] extern crate kg_utils;
/// # fn main() {
...
macro_rules! linkedhashmap {
    ($( $key: expr => $val: expr ), * $(,)*) => {{
         let mut map = $crate::collections::LinkedHashMap::new();
         $( map.insert($key, $val); )*
         map
    }}
}
...
impl<K: Hash + Eq, V, S: BuildHasher> IntoIterator for LinkedHashMap<K, V, S> {
    ...
    fn into_iter(mut self) -> IntoIter<K, V> {
        ...
        let map = mem::MaybeUninit::uninit();
        self.map = unsafe { map.assume_init() };
        mem::forget(self);
